// Fixed RV32IM Pipeline CPU
// Fixes the memory read timing issue

module rv32im_pipeline_fixed(
  input clk,
  input rst_n
);
  ...
  // Use register for instruction to avoid timing issues
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if_inst_reg <= paddr_read(pc, 4);
      
      // Update pipeline registers
      if_id_pc <= pc;
      if_id_inst <= if_inst_reg;
  ...
  always @(posedge clk) begin
    if (rst_n) begin
      ...
      if (cycle_count < 10) begin
        ...
        $display("  PC=%h, IF:inst=%h", pc, if_inst_reg);
        $display("  IF/ID: pc=%h, inst=%h", if_id_pc, if_id_inst);
        $display("  ID/EX: pc=%h, rd=%h, rs1_val=%h, rs2_val=%h", 
                 id_ex_pc, id_ex_rd, id_ex_rs1_val, id_ex_rs2_val);
        $display("  EX/MEM: pc=%h, alu=%h, rd=%h", 
                 ex_mem_pc, ex_mem_alu_result, ex_mem_rd);
        $display("  MEM/WB: pc=%h, data=%h, rd=%h", 
  ...
  initial begin
    $display("[PIPELINE] Fixed RV32IM Pipeline CPU initialized");
  end
