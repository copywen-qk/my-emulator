// Final RV32IM Pipeline CPU with proper timing
// This version should fix the pipeline timing issues

module rv32im_pipeline_final(
  input clk,
  input rst_n
);
  ...
  wire [31:0] inst_fetch = paddr_read(pc, 4);
  ...
  always @(posedge clk) begin
    if (ex_mem_valid) begin
      if (ex_mem_memtoreg) begin
        // Load operation
        mem_data_reg <= paddr_read(ex_mem_alu_result, 4);
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      mem_wb_pc <= ex_mem_pc;
      mem_wb_data <= ex_mem_memtoreg ? mem_data_reg : ex_mem_alu_result;
      mem_wb_rd <= ex_mem_rd;
  ...
  always @(posedge clk) begin
    if (mem_wb_valid && mem_wb_regwrite && (mem_wb_rd != 0)) begin
      ...
      $display("[WB] Write x%0d = %h", mem_wb_rd, mem_wb_data);
    end
  end
  ...
  always @(posedge clk) begin
    if (rst_n) begin
      ...
      // Detailed debug for first 10 cycles
      if (cycle_count < 10) begin
        $display("=== CYCLE %0d ===", cycle_count);
        $display("  PC: %h", pc);
        $display("  IF: inst=%h (valid=%b)", inst_fetch, if_id_valid);
        $display("  ID: pc=%h, inst=%h, rd=x%0d", if_id_pc, if_id_inst, rd);
        $display("  EX: pc=%h, alu=%h, rd=x%0d", id_ex_pc, alu_result, id_ex_rd);
        $display("  MEM: pc=%h, data=%h, rd=x%0d", ex_mem_pc, mem_data_reg, ex_mem_rd);
        $display("  WB: pc=%h, data=%h, rd=x%0d", mem_wb_pc, mem_wb_data, mem_wb_rd);
        $display("");
      end
      
      // Summary after pipeline is full
      if (cycle_count == 5) begin
        $display("[PIPELINE] Pipeline should be full now");
        $display("[PIPELINE] 5 instructions in flight:");
        $display("[PIPELINE]   IF: Instruction %h", inst_fetch);
        $display("[PIPELINE]   ID: Instruction %h", if_id_inst);
        $display("[PIPELINE]   EX: Instruction %h", id_ex_inst);
        $display("[PIPELINE]   MEM: Instruction %h", ex_mem_pc);
        $display("[PIPELINE]   WB: Instruction %h", mem_wb_pc);
      end
    end
  end
  
  initial begin
    $display("[PIPELINE] Final RV32IM Pipeline CPU initialized");
    $display("[PIPELINE] Expecting PC to increment each cycle");
  end
