...
#include "Vrv32im_pipeline_final.h"
...
int main(int argc, char** argv) {
    ...
    std::cout << "Starting RV32IM Final Pipeline CPU simulation..." << std::endl;
    std::cout << "Test image: " << argv[1] << std::endl;
    
    // Create the CPU instance
    Vrv32im_pipeline_final* cpu = new Vrv32im_pipeline_final;
    ...
    // Simulation loop
    int max_cycles = 10000;  // Reduced for testing
    ...
    if (instruction_count > 0) {
        std::cout << "Approximate CPI: " << (max_cycles / (float)instruction_count) << std::endl;
    }
